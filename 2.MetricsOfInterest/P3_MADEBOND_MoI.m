%% LEAP soc-nsoc videos Pipelines: get power and connectivity values from the data preprocessed with the MADE-BOND pipeline 
% P3 - pipeline 3 in the manuscript

% This script takes the data processed with the MADE-BOND pipeline and converges
...
% MADEBOND_preprocessing_report.mat file. Preprocessed data are saved. 
% The measures of interest are stored into a reporting table named
% MADEBOND_MOI_table and saved as MADEBOND_MeasuresOfInterest_report.mat. 
...
% Adapted by Rianne Haartsen, PhD.; 11-2022 
...
ft_defaults
addpath(genpath('xxx/Preproc_MADEBOND')) % any additional folders needed. 

load xxx/Preproc_MADEBOND/MADEBOND_preprocessing_report.mat % resulting report from the MADE-BOND pipeline with paths to pre-processed

Pow_location = 'xxx/Preproc_MADEBOND/A_Power_data';% folder where individual power data will be stored
FC_location = 'xxx/Preproc_MADEBOND/B_FC_data';% folder where individual connectivity will be stored
output_location = 'xxx/Preproc_MADEBOND';% location where the file with metrics of interest will be stored
Fig_location = 'xxx/Preproc_MADEBOND/Report_figs';% location where report figures will be stored (spectra and topoplots for theta and alpha frequency bands) 



for subject = 1:height(MADEBOND_report_table) 
    
    CurrID = MADEBOND_report_table.ID{subject};
    
    fprintf('\n\n\n*** Processing subject %d (%s) ***\n\n\n', subject, CurrID);
    
    try
        % load preprocessed data
        filename_path_preproc = MADEBOND_report_table.path_preproc{subject};
    ...
    MADEBOND_MOI_table(subject,:) = data_table_newrow;
    save('MADEBOND_MeasuresOfInterest_report.mat','MADEBOND_MOI_table');
    clear MADEBOND_MOI_table
