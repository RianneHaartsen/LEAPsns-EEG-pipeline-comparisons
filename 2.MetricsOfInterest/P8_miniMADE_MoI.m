%% LEAP soc-nsoc videos Pipelines: get power and connectivity values from the low-density data preprocessed with the miniMADE pipeline 
% P8 - pipeline 8 in the manuscript

% This script takes the low-density data (19 channels) processed with the 
% miniMADE pipeline and converges it to a fieldtrip structure. Then power 
...
% miniMADE_preprocessing_report.mat file. Preprocessed data are saved. 
% The measures of interest are stored into a reporting table named
% miniMADE_MOI_table and saved as miniMADE_MeasuresOfInterest_report.mat. 
...
addpath('/Users/riannehaartsen/Documents/MATLAB/eeglab2022.0_old');% enter the path of the EEGLAB folder in this line
eeglab nogui;% call eeglab to set up the plugin
addpath('/Users/riannehaartsen/Documents/MATLAB/fieldtrip-20180925');  % add fieldtrip path
% addpath('/Users/riannehaartsen/Documents/MATLAB/fieldtrip-20210629')
ft_defaults
addpath(genpath('/Users/riannehaartsen/Documents/MATLAB/LEAPsnsvids/LEAPsnsvids/Preproc_MATLAB'))

InfoPath = '/Users/riannehaartsen/Documents/02a_LEAP_EEG/LEAP_SocNSocVids/01_EEGdata/Pipeline_comparisons_subset/Preproc_miniMADE_v1_March2024';
...
% get IDs for input
FilesSet = dir(fullfile(strcat('/Users/riannehaartsen/Documents/02a_LEAP_EEG/LEAP_SocNSocVids/01_EEGdata/RawData_subset/'), '*.set'));
...
for subject = 1:height(FilesSet) 
    ...
    CurrID = CurrFileName(1:(strfind(CurrFileName,'_')-1));
    
    fprintf('\n\n\n*** Processing subject %d (%s) ***\n\n\n', subject, CurrID);
    
    ProcessedFileName = strcat(CurrID, '_social_nonsocial_videos_processed_data.set');
    
    if exist(strcat(InfoPath, '/processed_data/', ProcessedFileName), 'file') == 2

        try
            % load preprocessed data
            Processedfullfilepath = strcat(InfoPath, '/processed_data/', ProcessedFileName);
...
%% Additional check 
% For some raw datasets, there was no clean data in the pre-procesed
% folder.
% The following code puts NaN values into the measure of interest table for
% these datasets which will allow easier analysis in the next scripts. 
...
% addpath('/Users/riannehaartsen/Documents/MATLAB/fieldtrip-20210629')
ft_defaults
addpath(genpath('xxx/Preproc_miniMADE'))

InfoPath = 'xxx/Preproc_miniMADE';
...
for subject = 1:height(FilesSet) 
    ...
    end % end of if does not file exists
