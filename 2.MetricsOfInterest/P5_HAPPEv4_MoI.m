%% LEAP soc-nsoc videos Pipelines: get power and connectivity values from the data preprocessed with the HAPPEv4 pipeline 
% P5 - pipeline 5 in the manuscript

% This script takes the data processed with the HAPPE version 4 pipeline and converges
...
% HAPPEv4_MOI_table and saved as HAPPEv4_MeasuresOfInterest_report.mat. 
...
ft_defaults
addpath(genpath('xxx/Preproc_HAPPEv4'))

InfoPath = 'xxx/Preproc_HAPPEv4/';
...
for subject = 1:height(FilesSet)
    ...
    if exist(strcat(InfoPath, '/4 - processed/', ProcessedFileName), 'file') == 2
        ...
        cd(output_location)
        if exist(strcat(InfoPath, '/HAPPEv4_MeasuresOfInterest_report.mat'),'file')
        ...
        HAPPEv4_MOI_table(subject,:) = data_table_newrow;
        save('HAPPEv4_MeasuresOfInterest_report.mat','HAPPEv4_MOI_table');
        clear HAPPEv4_MOI_table
    else
        % Info
        Site{subject} = 'NA';
        DateCur{subject} = datetime('now');
