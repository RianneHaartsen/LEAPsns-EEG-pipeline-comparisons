%% LEAP soc-nsoc videos Pipelines: get power and connectivity values from the data preprocessed with the HAPPEv1 pipeline 
% P4 - pipeline 4 in the manuscript

% This script takes the data processed with the HAPPE version 1 (legacy) pipeline and converges
...
% HAPPE_preprocessing_report.mat file. Preprocessed data are saved. 
% The measures of interest are stored into a reporting table named
% HAPPEv1_MOI_table and saved as HAPPEv1_MeasuresOfInterest_report.mat. 
...
ft_defaults
addpath(genpath('xxx/Preproc_HAPPEv1'))

load xxx/Preproc_HAPPEv1/HAPPE_preprocessing_report.mat

Pow_location = 'xxx/Preproc_HAPPEv1/A_Power_data';
FC_location = 'xxx/Preproc_HAPPEv1/B_FC_data';
output_location = 'xxx/Preproc_HAPPEv1';
Fig_location = 'xxx/Preproc_HAPPEv1/Report_figs';


for subject = 1:height(HAPPE_report_table)
    
    CurrFileName = HAPPE_report_table.FileNames{subject};
    ID = CurrFileName(1:(strfind(CurrFileName,'_')-1));
    
    fprintf('\n\n\n*** Processing subject %d (%s) ***\n\n\n', subject, ID);
    
    if HAPPE_report_table.Number_Segments_Post_Segment_Rejection(subject) > 0 && ...
        ~isnan(HAPPE_report_table.Number_Segments_Post_Segment_Rejection(subject))
    
        try
            %% Step 1: load preprocessed data
            filename_path_preproc = strcat('xxx/Preproc_HAPPEv1/processed_data/', ...
                ID, '_social_nonsocial_videos_processed.set');
        ...
        %% Check data and create empty variables if not created
            ...
            if Neps_tot > 0
                ...
                if ~isempty(EEG.euaims.comments)
                ...
                % add info on ICs removed / interpolated channels
                subplot(4,5,[16 17])
                txt = 'Artifact information:';
                text(0.1,1,txt, 'FontSize', 16);
                hold on
                txt2 = sprintf('ICA: %i ICs out of %i total ICs removed', MADE_report_table.n_ICs_removed{subject}, ...
                    MADE_report_table.total_ICs(subject));
                text(0.2,.8,txt2, 'FontSize', 16);
                txt3 = sprintf('Epochs before vs after AR: %i - %i', MADE_report_table.total_epochs_before_artifact_rejection(subject), ...
                    MADE_report_table.total_epochs_after_artifact_rejection(subject));
                text(0.2,.6,txt3, 'FontSize', 16);
                txt4 = sprintf('Interpolated channels:  %i', MADE_report_table.n_total_channels_interpolated(subject));
                text(0.2,.4,txt4, 'FontSize', 16);
                txt5 = sprintf(' %s', convertCharsToStrings(MADE_report_table.label_total_channels_interpolated{subject})); 
                text(0.3,.2,txt5, 'FontSize', 14);
                set(gca,'YTickLabel',[]); set(gca,'XTickLabel',[]);
                set(gca,'ytick',[]); set(gca,'xtick',[]);
                xlim([0 3])
                ylim([0 1.1])
...
%         end
        ...
        HAPPEv1_MOI_table(subject,:) = data_table_newrow;
        save('HAPPEv1_MeasuresOfInterest_report.mat','HAPPEv1_MOI_table');
        clear HAPPEv1_MOI_table
    else
        % Info
        Site{subject} = 'NA';
        DateCur{subject} = datestr(now,'dd-mm-yyyy');
