%% Analysis of social and non-social videos in LEAP: Pipeline 5 and 8 - HAPPEv4 and HAPPILEE
% The HAPPE version 4 pipeline was adapted to ensure the pipeline was
% compatible with the EEG data format in the LEAP study. In addition, the
% segmenting was adapted for consistency with the other pipeline used in
% the current project (1-sec segments with 50% overlap).

% Input parameters used for pipeline 5: HAPPEv4 can be found in the folder:
% HAPPE-master-March2024/HAPPEv4_input_parameters_used.

% Input parameters used for pipeline 8: HAPPILEE can be found in the folder:
% HAPPE-master-March2024/HAPPEv4/input_parameters_used.
...
for currFile = 1:length(FileNames)
    ...
    %                 rethrow(ME) ;
            ...
            % RH: this step does not work if the EEG.data consists of
            % single instead of double, change to double type
        ...
        % compatible with the LEAP EEG data format. The current section
        % contains the segmenting by videos and segmenting into short
        % 1-second segments (as in the other pipelines).
        ...
        % LEAP site
            ...
            subject = currFile;
            FlashDatapath = 'xxx'; % RH; this variable is specific to the LEAP social/non-social videos dataset only
            ...
            samples_per_video = rest_video_length * EEG.srate;
             ...
             Subj_ID = extractBefore(FileNames{currFile},'_social_nonsocial_videos.set');
                Flash_name = fullfile(FlashDatapath,sprintf('Tablesocialnonsocial - %i.mat',str2double(Subj_ID)));
        ...
        % Create segment info for epoching
                ...
                EEG_afterEpoching_RH = EEG;


        %% original segmenting in HAPPE v4 pipeline

        % RH note: considering we had to adapt the pipeline to be
        % compatible with the LEAP EEG data, we do not need the
        % happe_segment function and therefore commented this out.
