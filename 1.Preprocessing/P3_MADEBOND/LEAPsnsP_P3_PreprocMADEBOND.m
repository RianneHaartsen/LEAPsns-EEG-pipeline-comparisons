%% Analysis of social and non-social videos in LEAP: Pipeline 3 - BOND-MADE
...
% Adapted by Rianne Haartsen, PhD.; 09-2022 
...
TimingsDatapath = 'xxx'; % RH note; this variable is specific to the LEAP social/non-social videos dataset only

% 2. Enter the path of the folder where you want to save the processed data
output_location = 'xxx/Preproc_MADEBOND';
...
for subject= 1:length(datafile_names)
    ...
    %    edit_event_markers_example() % an example of a labeling script from the appendix folder that can be called
        ...
        % RH note: in LEAP the REF channel is the same for all datasets
        ...
        % RH note; some of the datasets contained flat channels which would
        ...
        if numel(FlatbadChans)==EEG.nbchan || numel(FlatbadChans)+1==EEG.nbchan ...
                || numel(FlatbadChans) >= round((N_channels_analysed-1)/100*10) % RH note; 10% relative to the number of common channels
            all_chan_bad_Flat=1;
