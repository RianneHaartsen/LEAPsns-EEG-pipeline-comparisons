%% Analysis of social and non-social videos in LEAP: Pipeline 2 - MADE
% using the MADE pipeline with adaptations compatible with the LEAP EEG
% data formats. 

% Adapted by Rianne Haartsen, PhD.; 02-2022 
...
TimingsDatapath = 'xxx'; % RH note; this folder contains data to check the log of the events from Task Engine and is specific to the LEAP EEG data

% 2. Enter the path of the folder where you want to save the processed data
output_location = 'xxx/Preproc_MADE';
...
faster_bad_channels=cell(1, length(datafile_names)); % number of bad channel/s identified by faster
ica_preparation_bad_channels=cell(1, length(datafile_names)); % number of bad channel/s due to channel/s exceeding xx% of artifacted epochs
length_ica_data=[]; % length of data (in second) fed into ICA decomposition
total_ICs=[]; % total independent components (ICs)
ICs_removed=cell(1, length(datafile_names)); % number of artifacted ICs
...
for subject= 77:length(datafile_names)
    ...
    try
        ...
        load xxx/LEAPCommonChannels.mat
        EEG = pop_select(EEG, 'nochannel', find(~ismember(lower({EEG.chanlocs.labels}),lower(LEAPCommonChannels)))); % keep only channels that correspond to the 61 core channels 
        EEG = eeg_checkset(EEG);
    ...
    %    EEG = eeg_checkset( EEG );
        ...
        % RH note; LEAP comes with own channel locations in data, correct to the
...
%         end
        ...
        % RH note: this step is specific to the LEAP EEG data format
        ...
        EEG.preproc.rejected_channels.number = length(FASTbadChans);
        EEG.preproc.rejected_channels.labels = {EEG.chanlocs(FASTbadChans).labels};
        ...
        if length(total_channels_interpolated) < subject || isempty(total_channels_interpolated(subject))
            total_channels_interpolated(subject) = NaN;
        end 
    end
    ...
    report_table_newrow = table({ID}, datafile_name_preproc_done(1,subject), preproc_path(1,subject), date_preproc(1,subject), reference_used_for_faster(1,subject), faster_bad_channels(1,subject), ...
        ica_preparation_bad_channels(1,subject), length_ica_data(subject), total_ICs(1,subject), ICs_removed(subject), total_epochs_before_artifact_rejection(subject), ...
        total_epochs_after_artifact_rejection(subject), total_channels_interpolated(subject));
    report_table_newrow.Properties.VariableNames={'ID','datafile_names_preproc', 'path_preproc','date_preproc','reference_used_for_faster', 'faster_bad_channels', ...
